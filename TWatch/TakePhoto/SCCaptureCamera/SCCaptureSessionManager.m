...
- (void)takePicture:(DidCapturePhotoBlock)block {
    if(ApplicationDelegate.isInCamera == NO)
    {
        return;
    }
    AVCaptureConnection *videoConnection = [self findVideoConnection];
    ...
    @try {
        [_stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
            ...
            UIImage* croppedImage = [UIImage imageWithData:imageData];
            /*
...
#warning 相机大小
    ...
    @catch (NSException *exception) {
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"打开相机失败" message:@"请确保在隐私设置里面开启了本应用的相机权限" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        [alertView show];
        return;
    }
