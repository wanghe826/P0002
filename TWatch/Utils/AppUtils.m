...
+ (BOOL)isEmail:(NSString *)email{
    ...
    NSTextCheckingResult *isMatchEmail = [regexEmail firstMatchInString:email
                                                                options:0
...
+ (BOOL)isQQNum:(NSString *)qqNum{
    ...
    NSTextCheckingResult *isMatchQQ = [regexQQ firstMatchInString:qqNum
                                                          options:0
                                                            range:NSMakeRange(0, [qqNum length])];
    if (isMatchQQ) {
        return YES;
    }
    return NO;
...
+ (BOOL)isNumber:(NSString *)string{
    NSString *patternNum  = @"^[0-9]*$";
    NSError *error = NULL;
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:patternNum options:0 error:&error];
    
    NSTextCheckingResult *isMatchQQ = [regex firstMatchInString:string
                                                        options:0
...
+(NSString *)getCurrentLanguagesStr
{
    NSArray *languages = [NSLocale preferredLanguages];
    NSString *currentLanguage = [languages objectAtIndex:0];
    NSLog ( @"当前的系统语言是:%@" , currentLanguage);
    
    if([currentLanguage isEqualToString:@"en"])
    {
        currentLanguage = [currentLanguage stringByAppendingString:@"-CN"];
    }
